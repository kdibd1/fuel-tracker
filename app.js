 ...
 * - fill records
 * - service records
 * - vehicle dashboard rendering
 *
 * Charts, Kuttabul fuel prices and startup live
 * in their own files.
 */

const FT = window.FuelTracker = window.FuelTracker || {};

const $ = id => document.getElementById(id);

const CFG =
  window.FUEL_TRACKER_CONFIG || {};

const KEY =
  'fuelTracker.data.v2';

const OLD =
  'fuelTracker.entries.v1';

const OPS =
  'fuelTracker.pending.v1';
...
async function uploadVehicle(v) {
  ...
  if (services(v).length) {
    await send({
      ...
      path:
        '/service_entries?on_conflict=id',
 ...
 * IMPORTANT:
 *
 * Do NOT call reset() from
 * form.onreset.
 *
 * That caused the original
 * infinite reset loop where
 * entered numbers disappeared.
 */
form.onreset =
  () =>
    setTimeout(
...
/*
 * These are the deliberate
 * connections between files.
 *
 * Instead of every file dumping
 * random globals into window,
